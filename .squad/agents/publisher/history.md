...
- 2026-03-27T14:30:00Z — **Substack title/subtitle packaging review (IDENTIFIED)**: Discovered V3 publishing has duplicate title/subtitle in body. Draft markdown includes `# Title` (H1) + `*Subtitle*` (italic) lines; when `buildPublishPresentation()` calls `markdownToProseMirror(cleanDraft)`, the entire markdown (including meta lines) is converted to ProseMirror nodes, which appear in both preview HTML and Substack `draft_body`. Meanwhile, the API is also receiving `title` and `subtitle` as separate fields. Result: on Substack and preview, title appears twice (once in field, once in H1; subtitle missing from field, appears only in body italic). Solution documented in `.squad/decisions/inbox/publisher-substack-packaging-review.md`: extract title/subtitle with existing `extractMetaFromMarkdown()` function, convert only the clean body to ProseMirror, ensure preview and publish share identical packaging. No implementation required per task charter — decision ready for future action.
