...
- **Card-first Notes with prod URLs on stage (2026-03-18):** Posted 5 stage review Notes on nfllabstage linking to production published `/p/` URLs. The card-first body pattern (teaser paragraph + linked paragraph with ProseMirror link marks to prod `/p/` URL) is the correct mechanism for article card rendering. Note IDs: 229372212 (JSN), 229372239 (KC Fields), 229372275 (Denver), 229372305 (Miami), 229372344 (Witherspoon). Key insight: prod `/p/` URLs are discoverable via `GET /api/v1/archive` on the production domain; two stage-8 articles in pipeline.db had `substack_url=None` — the archive API is a reliable fallback for URL discovery. The `noteTextToProseMirror()` function does NOT add link marks; to get cards, the body must include explicit ProseMirror link marks via `buildCardFirstBody()` pattern. All 5 Notes recorded in `pipeline.db` notes table with target='stage'.
- **pipeline.db substack_url gap (2026-03-18):** Both stage-8 published articles (`witherspoon-extension-cap-vs-agent`, `seahawks-rb1a-target-board`) have `substack_url=None` despite being published. This means `record_publish()` was either not called or called without the URL. Recommend backfilling `substack_url` for published articles and auditing the publish workflow to ensure `record_publish()` always captures the canonical URL.
- **noteTextToProseMirror link-mark fix (2026-03-18):** Root cause of link-vs-card behavior confirmed: `noteTextToProseMirror()` emitted plain text paragraphs with no ProseMirror link marks. Substack requires explicit `marks: [{ type: "link", attrs: { href } }]` on URL text nodes to trigger article card rendering. Fix: added `parseNoteInline()` helper that auto-detects `https://` URLs and wraps them in link marks. The 5 Phase 4 Notes (229372212–229372344) were deleted and replaced with corrected versions: 229378039 (JSN), 229378074 (KC Fields), 229378102 (Denver), 229378151 (Miami), 229378200 (Witherspoon). pipeline.db `notes` table updated with new URLs.
- **substack_url backfill (2026-03-18):** Backfilled `substack_url` for both Stage 8 published articles: `seahawks-rb1a-target-board` → `https://nfllab.substack.com/p/the-6-million-backfield-how-seattle`, `witherspoon-extension-cap-vs-agent` → `https://nfllab.substack.com/p/cap-says-27m-the-agent-demands-33m-d00`. URLs confirmed via `GET /api/v1/archive` on nfllab.substack.com (production archive API).

## Phase 2Target Selection (2026-03-17T13:24Z)
...
📌 Team update (2026-03-17T15:59:35Z): Posted five card-first stage review Notes on nfllabstage pointing at production /p/ URLs; noted the Note IDs (229372212, 229372239, 229372275, 229372305, 229372344) and cleanup via delete-notes-api.mjs.
