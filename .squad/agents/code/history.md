...
- 2026-03-25 — Dashboard publish config UX: the publish/draft HTMX 500 came from the early `!substackService` guard in `src/dashboard/server.ts`, before the Substack sync try/catch runs. Keep JSON callers on 500, but for publish-panel HTMX actions return a swapped `renderPublishWorkflow()` fragment with concrete recovery steps (`SUBSTACK_PUBLICATION_URL`, `SUBSTACK_TOKEN`, restart dashboard, verify `/config`). Regression coverage lives in `tests/dashboard/publish.test.ts`.
...
- 2026-03-23T04:12:59Z — **UX Dashboard Publish Review findings integrated**: UX submitted read-only findings on dashboard publish/draft HTMX config error handling. **Key insight:** HTMX 500 responses do not swap the publish panel; operators see raw failure instead of recovery guidance. **Recommendation:** For HTMX publish-panel actions with missing `substackService`, return an HTML fragment from `renderPublishWorkflow()` with setup guidance (`SUBSTACK_PUBLICATION_URL`, `SUBSTACK_TOKEN`, restart dashboard, verify `/config`). Keep non-HTMX callers on JSON 500. Also reviewed and consolidated dashboard Substack service startup wiring issue: `createApp()` expects optional `substackService` but `startServer()` never constructs or wires it, causing normal startup to fail publish even when env vars are present. Consensus: treat publishing integrations as startup-wired optional services, not route-level env lookups. Route tests pass because they inject mocks directly; add startup-path coverage to catch wiring gaps. See decisions.md for full Substack runtime wiring decision chain (Code + Publisher + Code-review).
