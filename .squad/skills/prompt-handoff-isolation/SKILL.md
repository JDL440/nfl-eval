...
- `src/pipeline/actions.ts` switched Writer/Editor/Publisher to summary-only shared handoffs, while Writer revisions still receive the explicit current `editor-review.md` artifact and Editor receives the shared summary plus `buildEditorPreviousReviews()`.
