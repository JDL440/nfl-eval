...
- For content-page follow-through after the shared shell is stable, prefer route-local hooks over reopening `layout.ts`: add explicit article artifact wrappers, publish workflow hierarchy classes, and trace readability classes, then drive the mobile fix from `styles.css` plus focused route tests.
