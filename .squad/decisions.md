...
- **Data: Article history + duplicate-risk brief** (2026-04-03): Current code has durable per-article history in SQLite plus repo/file mirrors, but no dedicated "recent related articles" recommender for duplicate-angle avoidance. The practical duplicate check is still manual: use runtime article history when available, then team-filtered article lists / repo article files to inspect recent Seahawks themes before drafting new casual prompts. For Seattle specifically, mid-March repo artifacts already cover Puka-vs-Seattle structural weakness, Emmanwori/chess-piece validation, JSN extension timing, Witherspoon extension economics, and RB/draft-board debate. A Tuesday casual prompt should avoid another "explain one existing roster flaw" version of those. See `.squad/agents/data/history.md` → Seahawks Theme Cluster for full artifact listing.

- **Decision: Artifact Editing + Feedback Loop Design** (2026-04-02): PENDING TEAM REVIEW. Design allows users to edit artifacts directly in dashboard, send corrective feedback to agents, and track edits/responses. Core components: artifact_edits table (audit trail), artifact_feedback_packets table (grouping), updated regress flow bundling edits + feedback together. Phases: Phase 1 (Foundation): schema + edit routes + simple UI; Phase 2 (Bundling): feedback packets + regress integration; Phase 3 (Injection): agent context + acknowledgment loop; Phase 4 (Polish): versioning UI, metrics, docs. Non-binding decisions: user edits are suggestions (agents free to ignore), feedback bundled with regression (not separate system), backward-compatible (no breaking schema changes). Acceptance criteria and implementation owner documented in `.squad/decisions/inbox/research-artifact-feedback-design.md`. Next step: Code Agent architecture review & Phase 1 scope sign-off.

- **UX: New Idea advanced section** (2026-04-02): Collapse advanced editorial controls (depth, format, model, provider selection) into a single bottom-of-form collapsible section. Keep default idea intake focused on prompt, teams, preset, pinned agents, and auto-advance. Preserve less-common overrides while keeping the form uncluttered. Constraint kept: The provider/model selector stays outside the override-disabled controls inside the advanced section so existing request payload behavior remains unchanged. Rationale: Default intake should guide editors toward preset-first thinking; advanced toggles are available but not cognitive load in the happy path. Tests: `tests/dashboard/new-idea.test.ts` passing. Build passed. Files: `src/dashboard/views/new-idea.ts`.
