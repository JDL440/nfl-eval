...
|--------|-----------------|--------|
| **nflverse** (primary) | Play-by-play (372 cols: EPA, WPA, CPOE), player/team stats, Next Gen Stats, snap counts, PFR advanced stats, FTN charting, combine, contracts, draft picks | ✅ **Local parquet cache + MCP tools** — see `~/.nfl-lab/agents/skills/nflverse-data.md` |
| **Polymarket** | Prediction market odds — Super Bowl futures, division winners, win totals, playoff odds, game outcomes, MVP | ✅ **Free API + MCP tool** — see `~/.nfl-lab/agents/skills/prediction-market-data.md` |
