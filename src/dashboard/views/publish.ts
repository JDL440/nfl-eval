  ...
  const content = `
    <div class="article-detail">
      <div class="detail-header">
        ...
        <p class="hint">Save a Substack draft for review, or publish the current article live in one step.</p>
      </div>
      ...
      <div class="detail-grid">
        ...
        <div class="detail-sidebar">
          ${renderPublishWorkflow({ article })}

          ${renderNoteComposer(article)}
          ${renderTweetComposer(article)}

          ${renderPublishAll(article.id)}
        </div>
      </div>
    </div>`;
...
export function renderPublishWorkflow(data: PublishResultData): string {
  const { article, success, message, messageTone, draftUrl, publishedUrl, error } = data;
  ...
  const tone = success === false ? 'error' : (messageTone ?? 'success');
  ...
  const alertHtml = hadAction && resolvedMessage
    ? `<div class="alert alert-${escapeHtml(tone)}">
         <strong>${success === false ? 'Action needed:' : 'Ready:'}</strong> ${escapeHtml(resolvedMessage)}
  ...
  const draftCopy = hasDraft
    ? `Latest draft saved to Substack. You can update it again or publish the current article live.`
    : `No Substack draft exists yet. Save one for review, or publish now to create a draft and go live in one step.`;
  ...
  return `
    <section id="publish-workflow" class="detail-section">
      ...
      <p class="hint">“Publish Now” always sends the latest article body. If a draft already exists, it is updated first and then published.</p>
    </section>`;
}
...
export function renderPublishAll(articleId: string): string {
